...
router.patch('/:id', (req: Request, res: Response) => {
  ...
  // policy blocks connections, not custody of the row. Reads the post-lock-strip
  // `fields`, so under the FXNet lock (host already removed) this is a no-op and
  // the allowlist only bites on non-locked instances.
  const nextHost = fields.host;
  if (
