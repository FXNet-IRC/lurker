...
export function openDccListener(opts: DccListenOptions = {}): Promise<DccListenHandle> {
  ...
  // Rotate over INDICES, not port numbers. Offsetting a port value only works
  // when the free set is contiguous — with a gap, `indexOf` misses and `slice`
  // silently collapses the list to its last element.
  const offset = Date.now() % free.length;
  const candidates = free.slice(offset).concat(free.slice(0, offset));
