...
class IrcManager extends EventEmitter {
  ...
  startNetwork(
    ...
    // FXNet WEBIRC: stamp the latest browser IP onto the row BEFORE the gate
    // reads it, so the freshly built connection carries it into WEBIRC on
    // connect (real-IP path). connectGate calls getNetwork internally, so this
    // has to land first for the fresh IP to reach the connection. The pause and
    // instance-lockdown checks that used to sit here now live in connectGate.
